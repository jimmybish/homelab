...
tools: ['execute/getTerminalOutput', 'execute/runInTerminal', 'read/terminalLastCommand', 'read/terminalSelection', 'edit', 'search', 'context7/*', 'todo', 'read/problems', 'web', 'web/fetch']
---
...
If a task hits issues, requires workarounds, or reveals missing knowledge, update this agent config (`.github/agents/proxmox_admin.agent.md`) or the relevant skill in `.github/skills/` so future runs are smoother.
